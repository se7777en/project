...
function Forms(formsSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      // u kajdoi knopki(sushnosti) gde est input est submit
      e.preventDefault();
...
async function GetResource(url) {
  const res = await fetch(url);
  if (!res.ok) {
    // u fetcha est OK - eto esli vse normalno i status - poluchaem status
...
(__webpack_require__(/*! es6-promise */ "../../node_modules/es6-promise/dist/es6-promise.js").polyfill)(); // dlya standarta es6
 // polifil dlya foreach
...
window.addEventListener('DOMContentLoaded', () => {
  const modalTimerId = setTimeout(() => (0,_modules_Modals__WEBPACK_IMPORTED_MODULE_2__.openModal)('.modal', modalTimerId), 50000);
  ...
  (0,_modules_Timer__WEBPACK_IMPORTED_MODULE_3__["default"])('.timer', '2023-01-05 00:00:00');
