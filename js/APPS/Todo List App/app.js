...
const loadTodos = (filter, filteredTodos) => {
    ...
    if (todosList.length === 0) {
        ...
        const emptyListString = filter ? `No ${filter} todo's!` : `No todo's added!`;
...
const sortTodos = (filteredTodos) => {
    ...
    todoList.sort((a, b) => {
        //compare the states
        if (a.state === b.state) {
            //if the state are the same, compare the dates
            return new Date(a.date) - new Date(b.date);
        } else {
            // if the states are the different, completed task come last
...
const toggleTodoState = (todoItem) => {
    const todo = todoListArray.find((todo) => todo.id === Number(todoItem.dataset.id));
