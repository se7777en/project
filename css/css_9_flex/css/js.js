		...
		function() {
			...
			module.exports = function(config) {
				...
				function maxDelta(value, measurements) {
					...
					for(measurementInd = 0; measurementInd < measurements.length; ++measurementInd)(delta = 100 * Math.abs(measurements[measurementInd].speed - value) / value) > maxDelta && (maxDelta = delta);
					return maxDelta
				}

				function isCompleted(metrics) {
					var maxInd, testTime = void 0 !== metrics.testTime ? metrics.testTime : timer() - startTime,
						measurements = metrics.progressMeasurements,
						afterCompleteDuration = metrics.afterCompleteDuration,
						numMeasurements = measurements.length,
						curSpeed = metrics.speed;
					return void 0 === afterCompleteDuration ? testTime >= maxDuration || !(numMeasurements < minStableMeasurements) && (maxInd = lastWindowMaxInd(measurements, Math.ceil(minStableMeasurements / 2)), !(numMeasurements - maxInd < Math.ceil(minStableMeasurements / 2)) && (maxInd = Math.max(0, numMeasurements - minStableMeasurements), !(numMeasurements - maxInd < minStableMeasurements) && (!(maxDelta(curSpeed, measurements.slice(numMeasurements - minStableMeasurements, numMeasurements)) > stabilityDelta) && testTime >= minDuration))) : testTime >= afterCompleteDuration
				}
			...
			module.exports = function() {
				var timer;
				return this.window && window.performance && (timer = window.performance.now || window.performance.webkitNow) && (timer = timer.bind(window.performance)), timer || (timer = function() {
				...
				return function(requester, config) {
					...
					function startTestWorker(workerId, url, attempt) {
						...
						function sendDownloadRequest(onDownloadProgress, onDownloadComplete, maxPayloadSize) {
							testAttempt < 3 || config.maxBytesInFlight - 1 < activeWorkersCount * maxPayloadSize || !req.supportsProgress() ? rangeUrl = url.replace("/range/", "/range/0-" + requestSize) : (rangeUrl = url.replace("/range/", ""), requestSize = MAX_PAYLOAD_BYTES), event(events.events.CONNECTION_START, {
								oca: oca,
								range: requestSize,
								requestUrl: rangeUrl
							});
							...
							var maxPayloadSize = MAX_PAYLOAD_BYTES,
								completeFunc = function(data) {
									var requestTime, eventData = {
										...
										partialSuccess = !1,
										ocaRequest = ocaCount[oca] || 0;
									if(ocaCount[oca] = ocaRequest + 1, maxPayloadSize = MAX_PAYLOAD_BYTES - ocaRequest, workerIsActive() && isActive() && "stopped" !== data.reason) {
										...
										eventData.bytes = requestSize, eventData.partialSuccess = partialSuccess, event(events.events.CONNECTION_SUCCESS, eventData), eventData.result = "success", req.supportsProgress() ? requestSize = maxPayloadSize : (requestTime = data.end - data.start, requestSize = Math.min(Math.floor(requestTimeMs * requestSize / requestTime), 5 * requestSize, maxPayloadSize, Math.round(config.maxBytesInFlight / config.connections.max)), requestTimeMs = Math.min(requestTimeMs + config.rangeRequestIncreaseMs, config.maxRangeRequestTimeMs), requestSize = Math.max(requestSize, 1024)), event(events.events.CONNECTION_END, eventData), sendRequest(onProgress, completeFunc, maxPayloadSize)
						...
						var rangeUrl, req = requester(),
							pinger = requester(),
							requestSize = config.firstRequestBytes,
							oca = url.split("/")[2];
						requesters[workerId] = req, pingers[workerId] = pinger, workerMeasurements[workerId] = [], latencyMeasurements[workerId] = [], req.supportsProgress() && (requestSize = MAX_PAYLOAD_BYTES), config.measureLatency && currentTestType !== TEST_TYPE.LATENCY_TEST && sendLatencyRequest(onLatencyComplete), currentTestType === TEST_TYPE.DOWNLOAD_TEST ? sendDownloadRequest(onProgress, onComplete(sendDownloadRequest), MAX_PAYLOAD_BYTES) : currentTestType === TEST_TYPE.UPLOAD_TEST ? sendUploadRequest(onProgress, onComplete(sendUploadRequest), MAX_PAYLOAD_BYTES) : currentTestType === TEST_TYPE.LATENCY_TEST && sendLatencyRequest(onLatencyComplete)
					}
					...
					function reportMetrics(interval) {
						...
						if(isActive())
							if(currentTestType === TEST_TYPE.LATENCY_TEST) resultData = computeUnloadedLatency(), resultData.stable = !0, testComplete = testTime() > 5 && resultData.count > 50, testComplete ? (stop(), resultData.result = "success", setTimeout(function() {
								...
								}, testComplete ? (testTimeS > config.duration.max && (resultData.stopperStable = !1, progressMeasurements.length > 10 && (resultData.stable = !0)), stop(), resultData.result = "success", setTimeout(function() {
									reportEndEvents(resultData)
								...
								if(activeWorkersCount < config.connections.max)
									for(currentSpeed > 5e7 ? desiredWorkers = config.connections.max : currentSpeed > 1e7 && activeWorkersCount < 5 ? desiredWorkers = 5 : currentSpeed > 1e6 && activeWorkersCount < 3 && (desiredWorkers = 3), currentSpeed > 5e5 && activeWorkersCount < 2 && (desiredWorkers = 3), i = 0; i < desiredWorkers - activeWorkersCount; ++i) startNewWorker()
							}
					}
					...
					function test(attempt, error) {
						...
						var eventData;
						if(reset(), (testAttempt = attempt || 1) > config.maxAttempts) return void reportEndEvents({
							result: "fail",
							error: error
						});
						...
						}, event(events.events.ATTEMPT_FAIL, eventData), eventData.result = "fail", event(events.events.ATTEMPT_END, eventData), error.url && urlGetter.reportBadUrl(error.url)), testAttempt > 1 ? setTimeout(startTest, Math.min(Math.pow(2, testAttempt), 200)) : startTest()
					}
					...
					return config = testConfig.get(config), timer = config.timer, speedAggregator = config.aggregator, stopper = config.stopper, that = {
						...
						off: function(eventName, callback) {
							...
							if(handlers && handlers.length > 0) {
								for(eventInd = 0; eventInd < handlers.length; ++eventInd)(event = handlers[eventInd]) !== callback && outHandlers.push(event);
			...
			utils.getXHR = function() {
				...
				var l, half;
				if(0 !== (l = values.length)) return 1 === l ? values[0] : (half = Math.floor(l / 2), values.sort(), l % 2 ? values[half] : (values[half - 1] + values[half]) / 2)
			...
			module.exports = function() {
				function positiveNumberValidator(name) {
					return function(value) {
						if(value <= 0) throw new errors.TesterArgumentError(name + ": expected value should be above 0, given value: " + value)
					}
				}
				...
				var errors = require("./error"),
					...
					validators = {
						...
						duration: function(duration) {
							...
							if(duration.max > 300) throw new errors.TesterArgumentError("duration.max is limited at 300")
						},
						connections: function(connections) {
							...
							if(connections.max > 30) throw new errors.TesterArgumentError("connections.max is limited at 30")
						}
					},
					defaultDuration = {
						min: 5,
			...
			module.exports = function(config) {
				...
				function isCompleted(metrics) {
					...
					if(void 0 === afterCompleteDuration) {
						...
						if(numMeasurements < minStableMeasurements) return !1;
						var maxInd = lastWindowMaxInd(measurements, minStableMeasurements);
						if(numMeasurements - maxInd < minStableMeasurements) return !1;
			...
			function CircularBuffer(capacity) {
				...
				if("object" == typeof capacity && Array.isArray(capacity._buffer) && "number" == typeof capacity._capacity && "number" == typeof capacity._first && "number" == typeof capacity._size)
					...
					if("number" != typeof capacity || capacity % 1 != 0 || capacity < 1) throw new TypeError("Invalid capacity");
			...
			CircularBuffer.prototype = {
				...
				get: function(start, end) {
					...
					if("number" != typeof start || start % 1 != 0 || start < 0) throw new TypeError("Invalid start");
					...
					if("number" != typeof end || end % 1 != 0 || end < 0) throw new TypeError("Invalid end");
			...
			var urlGetter = function() {
				...
				function reset() {
					testUrls = [], curUrlInd = 0, req = void 0, waitOnRefresh = !1
				}
				...
				return errors = require("./error"), dummy = require("./utils").dummy, DEFAULT_PARAMS = {
				...
				}(), reset(), that.get = function(urlParams, requester, onComplete, onFail, onStop, refresh) {
					...
					if(0 === testUrls.length || refresh) return void that.refresh(urlParams, requester, returnUrl, onFail, onStop);
					returnUrl(testUrls)
				}, that.getNext = function(urlParams, requester, onComplete, onFail, onStop, refresh) {
					function wrapper(func) {
						return function(data) {
							waitOnRefresh = !1, func(data)
						}
					}
					...
					function getUrl() {
						curUrlInd = (curUrlInd + 1) % testUrls.length, testUrls.length > 0 ? onComplete(testUrls[curUrlInd]) : onFail(new errors.NoOcaUrlsError("No urls returned from the endpoint " + urlParams))
					}
					testUrls.length < 3 || refresh ? waitOnRefresh ? waitTillRefresh(getUrl) : (waitOnRefresh = !0, that.get(urlParams, requester, wrapper(onComplete), wrapper(onFail), wrapper(onStop), !0)) : getUrl()
			...
			module.exports = function() {
				...
				function getLatestSnapshotMeasurements(workerMeasurements) {
					...
					for(workerId in workerMeasurements)
						if(workerMeasurements.hasOwnProperty(workerId)) {
							if(curWorkerMeasurements = workerMeasurements[workerId], workerMeasurementsCount = curWorkerMeasurements.length, lastMeasurementInd = lastProcessedWorkerMeasurement[workerId] || 0, curWorkerOverflowMeasurements = overflowMeasurements[workerId] || [], !(lastMeasurementInd < workerMeasurementsCount || curWorkerOverflowMeasurements.length > 0)) return;
				...
				function makeSnapshot(snapshotMeasurements) {
					...
					if(void 0 !== (maxTime = curLastTime) && void 0 !== minTime) return snapshot.start = minTime, snapshot.end = maxTime, snapshot
				}
			...
			var requester = function() {
				...
				function uploadTest(url, size) {
					...
					var lastBytesLoaded, lastProgressTime, lastCompleteTime, xmlHttpTimeout;
					lastBytesLoaded = 0, request = xhr(), request.upload && (request.upload.onprogress = onprogress), xmlHttpTimeout = setTimeout(ontimeout, requestTimeoutMs), "onreadystatechange" in request ? request.onreadystatechange = function(e) {
				...
				return requestTimeoutMs = 1e4, errors = require("../error"), utils = require("../utils"), xhr = utils.getXHR, timer = require("../timer").time, dummy = require("../utils").dummy, testIsRunning = !1, {
					...
					supportsProgress: function() {
						if(void 0 !== supportsProgress) return supportsProgress;
			...
			function logger(tester, config) {
				function sendLogs(data) {
					...
					request.onload ? (request.onload = onload, request.onerror = onerror) : request.onreadystatechange = function(e) {
				...
				function endConnectionSession(requestUrl, data) {
					...
					data.type = endSessionType(data), data.measurements = connectionEvents[requestUrl], nfLogger.endSession(connectionSessionId, data), delete connectionSessionIds[requestUrl], delete connectionEvents[requestUrl]
				}
				...
				PAGE_LOAD = "pageLoad";
				var logUrl, logSource, nfLogger, testSessionId, testAttemptSessionId, afterCompleteSessionId, loggingEnabled = !1,
				...
				return config = config || {}, logUrl = config.url || "https://ichnaea.test.netflix.com/cl2", logSource = config.source || "netspeed", require("./utils"), nfLogger = require("nf-cl-logger")({
					...
					startLogging: function() {
						loggingEnabled || (loggingEnabled = !0, tester.on(events.START, function(data) {
							...
							connectionSessionIds[request] = nfLogger.startSession("Connection", data), connectionEvents[request] = []
			...
			Logger.prototype = {
				...
				_initProperties: function(options) {
					for(var option in this) "function" != typeof this[option] && options && "_" !== option.charAt(0) && (void 0 !== options[option] ? this[option] = options[option] : this[option] = this[option])
				},
				...
				_initContext: function(type, data) {
					...
					return context = data ? this._copyData(data) : {},
						SCHEMA && SCHEMA.types[type[0]] ? context.type = SCHEMA.types[type[0]] : context.type = type, context.id = this._getNextContextId(), context
				},
				_initEventContext: function(type, data) {
					var context = this._initContext(type, data);
					return context.sequence = ++this._state.sequenceNumber, void 0 === context.time && (context.time = this._timestamp()), context
				},
			...
			function convertSpeed(result) {
				var bitsPerS = result.speed,
					...
					units = "Mbps";
				return speedMbs < 1 ? (speedMbs *= 1e3, units = "Kbps") : speedMbs >= 995 && (speedMbs /= 1e3, units = "Gbps"), speedMbs = speedMbs < 9.95 ? (Math.round(10 * speedMbs) / 10).toFixed(1) : speedMbs < 100 ? Math.round(speedMbs) : 10 * Math.round(speedMbs / 10), {
					speed: speedMbs,
					units: units
				}
			}
			...
			var TEST_CONFIG = {
				...
				ui = function(tester, logger) {
					...
					function setupLanguageControls() {
						...
						for(elemInd = 0; elemInd < languageOptions.length; ++elemInd) elem = languageOptions[elemInd],
							function(elem) {
					...
					var showMoreDetails = function() {
						...
						cancelConfig = function() {
							hideElement(configContainerElem), !0 === config.showAdvanced || "true" === config.showAdvanced ? showMoreDetails() : hideMoreDetails()
						},
						applyConfig = function() {
							...
							for(var name in TEST_CONFIG) {
								...
								testerValue && ("boolean" == typeof TEST_CONFIG[name].default ? config[name] = testerValue.checked : config[name] = testerValue.value, shouldPersist && canUseLocalStorage && (window.localStorage[name] = String(config[name])))
							}
							try {
								applyTesterConfig(config), hideElement(configContainerElem), !0 === config.showAdvanced || "true" === config.showAdvanced ? showMoreDetails() : hideMoreDetails(), restartTest()
							} catch(e) {
						...
						that = {
							onProgress: function(result) {
								var updateMetric = function(testType) {
									...
									"download" == testType ? (speedElemId = "speed-value", bytesElemId = "down-mb-value", speedUnitId = "speed-units", convertFunc = convertSpeed) : "upload" == testType ? (speedElemId = "upload-value", bytesElemId = "up-mb-value", speedUnitId = "upload-units", convertFunc = convertSpeed) : "latency" == testType ? (speedElemId = "latency-value", speedUnitId = "latency-units", convertFunc = convertLatency) : "bufferbloat" == testType && (speedElemId = "bufferbloat-value", speedUnitId = "bufferbloat-units", convertFunc = convertLatency), speedElem = getElement(speedElemId), speedUnitsElem = getElement(speedUnitId), bytesElemId && (bytesElem = getElement(bytesElemId), bytesElem.innerHTML = convertToMB(result)), speedData = convertFunc(result), speedData.speed, speedData.units, speedElem && (speedElem.innerHTML = speedData.speed), speedUnitsElem && (speedUnitsElem.innerHTML = speedData.units)
								};
								updateMetric(result.testType), "download" === result.testType && updateMetric("bufferbloat"), "true" !== config.measureUploadLatency && !0 !== config.measureUploadLatency || "upload" !== result.testType || updateMetric("bufferbloat")
							},
			...
			! function(root, factory) {
				...
				function getReplacedViewportUnits() {
					...
					return declarations.forEach(function(item) {
						...
						_open && !open && (open = _open, close = _close), buffer.push(_item.content)
			...
			ui = require("./ui")(tester, testLogger), tester.on(events.START, function() {
				...
				}), logger.startLogging(), ui.setupEvents(),
				function() {
					tester.download()
