    ...
    if (currentUrl.indexOf('prolingo.surge.sh') === -1) {
        if (currentUrl.indexOf('prolingo.surge.sh') === -1) {
            ...
            closeIcon.addEventListener('click', () => {
                ...
                identifer = false;
                replyInner.classList.add('pulse');
            });
            ...
            replyInner.addEventListener('click', () => {
                ...
                wrongAnswers = 0;
                identifer = true;
                replyInner.classList.remove('pulse');
            });
            ...
            let clicked = false; // Переменная для отслеживания того, была ли нажата кнопка за последние 5 секунд

            onlickBtbParent.addEventListener('click', (e) => {
                if (identifer) {
                    if (!clicked && e.target.closest('.wrapper__box-question')) { // Проверяем, была ли кнопка нажата за последние 5 секунд
                        clicked = true; // Устанавливаем в true, чтобы предотвратить нажатие кнопки в течение 5 секунд
                       ...
                       // console.log(event);
                        ...
                        setTimeout(() => {
                            clicked = false; // Устанавливаем в false, чтобы снова разрешить нажатие кнопки через 5 секунд
