...
class ColorOne extends Component {
    ...
    render() {
        ...
        const {colorOne, getColorIntensity} = this.props;
        // console.log(colorOne)

        const textColor = getColorIntensity(colorOne) !== 'light' ? '#000000' : '#ffffff';
        ...
        return (
            <div className="main__item one" style={{backgroundColor: colorOne, color: textColor}}>
                <div className="color__descr">
                    <div className={itemColor} onClick={() => this.setTemporaryState('colorActive')}>
                        <div className="color">{colorOne}</div>
                    </div>
